...
import { secctionCopy, modalssss, userInfo, setting } from "@/constant";
function App() {
  const emailLink = `mailto:${setting[0].email}?subject=Hi,+setting`;
  const websitelink = `https://${setting[0]["website"]}`;
    ...
    <>
      <div className="mx-auto space-y-4 px-4 md:max-w-2xl md:px-0">
        <header className="-mx-2 mt-2 md:mx-0">
          ...
          <Image alt="Le Vinh Khang's cover image" decoding="async" data-nImage="fill" className="object-cover" width={100} height={100} style={{position:"absolute",height:"100%",width:"100%",inset:0,color:"transparent",}}sizes="(min-width: 768px) 672px, calc(100vw - 16px)" src={userInfo[0]["background"]}/>
          </div>
        ...
        <main className="space-y-4">
          ...
          <div className="sticky bottom-1 z-10 grid grid-cols-2 gap-2 rounded-xl p-2 border border-slate-700 bg-slate-800/80 backdrop-blur-md shadow-[0_0_8px_4px_rgba(0,0,0,0.25)]">
            <button
              type="button"
              className="flex items-center justify-center space-x-2 rounded-lg px-4 py-2  bg-slate-700 text-white transition-colors select-none group hover:bg-blue-vani"
              onClick={() =>
            ...
            <button
              ...
              className="flex items-center justify-center space-x-2 rounded-lg px-4 py-2  bg-slate-700 text-white transition-colors select-none group hover:bg-blue-vani"
              onClick={() => {
                const fileUrl = "vani.vcf";
                const fileName = "vani.vcf";
        ...
        <footer className="space-y-4 pb-8 pt-4">
          <div className="flex flex-col items-center space-y-3">
            <span className="select-none text-sm leading-none text-slate-400">
              © {/* */}2024{/* */} setting. All rights reserved.
            </span>
          </div>
        </footer>
      </div>
      ...
      <div id="imageContainer" style={{ display: "none" }}>
        {/* <Image id="myImage" src="https://Image.vietqr.io/image/MB-0905243477-print.png" alt="Image" /> */}
      </div>
      <Toaster
      />
      {modalssss.map((item, index) => (
